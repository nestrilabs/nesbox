...
pub struct Worker {
    ...
    mem: GuestMemoryMmap,
    /// The CTL queue shared with the fence-handler closure inside VirtioGpu.
...
impl Worker {
    ...
    fn work(mut self) {
        ...
        loop {
            ...
            if queue_index == CTL_INDEX {
                if self.process_ctl_queue(&mut virtio_gpu) {
                    // At least one descriptor was retired; notify the driver.
                    if let Err(e) = self
                        .interrupt
                        .trigger(VirtioInterruptType::Queue(CTL_INDEX as u16))
                    {
                        error!("virtio-gpu: failed to signal CTL queue interrupt: {e:?}");
                    }
                }
            }
    ...
    fn process_ctl_queue(&mut self, virtio_gpu: &mut VirtioGpu) -> bool {
        ...
        loop {
            ...
            let head = {
                let mut queue = self.queue_ctl.lock().unwrap();
                match queue.pop() {
                    Ok(Some(head)) => head,
                    Ok(None) => break,
                    Err(e) => {
                        // Guest sent more descriptors than queue size – fatal.
                        panic!("virtio-gpu: invalid avail idx in CTL queue: {e:?}");
                    }
                }
            };
            ...
            if writer.available_bytes() == 0 {
                let mut queue = self.queue_ctl.lock().unwrap();
                if let Err(e) = queue.add_used(desc_index, 0) {
                    error!("virtio-gpu: add_used error: {e:?}");
                }
