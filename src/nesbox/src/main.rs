...
const DEFAULT_API_SOCK_PATH: &str = "/run/nesbox.socket";
const VMM_VERSION: &str = if cfg!(feature = "fuzzing") {
...
fn main() -> ExitCode {
    ...
    if let Err(err) = result {
        ...
        error!("Nesbox exiting with error. exit_code={exit_code}");
        ExitCode::from(exit_code)
    } else {
        info!("Nesbox exiting successfully. exit_code=0");
        ExitCode::SUCCESS
    }
}
...
fn main_exec() -> Result<(), MainError> {
    ...
    // Start nesbox by setting up a panic hook, which will be called before
    ...
    info!("Running Nesbox v{VMM_VERSION}");

    #[cfg(feature = "fuzzing")]
    warn!(
        "This Nesbox binary was built with the `fuzzing` feature enabled. This disables \
...
/// RLIMIT_NOFILE is set (this can only happen if nesbox is run outside the jailer. 2048 is
...
fn resize_fdtable() -> Result<(), ResizeFdTableError> {
    ...
    // nesbox will not need to reallocate it later. If the file descriptor table
...
fn run_without_api(
    ...
    // Create the nesbox metrics object responsible for periodically printing metrics.
    let nesbox_metrics = Arc::new(Mutex::new(metrics::PeriodicMetrics::new()));
    event_manager.add_subscriber(nesbox_metrics.clone());
